...
public class XSSFWorkbookParser {
    ...
    /*private void openWorkbook(){
        ...
        try{
            //TODO: change to windows File 'file'
            workbook = new XSSFWorkbook(new FileInputStream(linuxFile));
        }catch (Exception e) { e.printStackTrace(); }

        //something open 'somepath + "hcs" + monday + ".xlsx"'
        prepareSheets();
    }
    //Save with day name daysByName[dayByInt]
    private void saveWorkbook(int dayByInt){
        try {
            String fileName = linuxFile .getAbsolutePath();
            FileOutputStream workbookWrite = new FileOutputStream(fileName.substring(0, fileName.length() - 5) +
                    daysByName[dayByInt] + ".xlsx");
            workbook.write(workbookWrite);
        }catch (Exception e){
            e.printStackTrace();
        }
    }
    //delete not used sheets
    private void prepareSheets(){
        for(String delete: sheetsToDelete)
            workbook.removeSheetAt(workbook.getSheetIndex(delete));
    }
    ...
    private void sheetIterator(int day){
        for(int z = 0; z < workbook.getNumberOfSheets(); z++){
            XSSFSheet sheet = workbook.getSheetAt(z);
            //TODO: set both offsets respectively
            if(specialSheets.contains(sheet.getSheetName())){
                deleteLoop(sheet, specialOffset, 3);
            }else if(brksnkSheets.contains(sheet.getSheetName())) {
                deleteLoop(sheet, brksnkOffset, day);
            }else{
                deleteLoop(sheet, lunchOffset, day);
            }

        }
    }

    private void deleteLoop(XSSFSheet sheet, int[] array, int day){
        for(int y = 0; y < array.length; y++) {
            //sheet.removeMergedRegion(array[y] - y);
            if (y == day)
                continue;
            for (int z = 0; z < 3; z++) {
                deleteColumn(sheet, array[y] + z);
            }
        }
    }
    //List<Integer> mergedRegions
    //sheet.removeMergedRegions(mergedRegions);
    ...
    private void deleteColumn(Sheet sheet, int column){
        ...
        do{
            ...
            if(lastColumn < column)
                continue;
            for(int z = column + 1; z < lastColumn + 1; z++){
                int y = z;
                if(row.getRowNum() == 0)
                    y++;
                Cell oldCell = row.getCell(z - 1);
                if(oldCell  != null)
                    row.removeCell(oldCell);
                Cell nextCell = row.getCell(y);
                if(nextCell != null){
                    Cell newCell = row.createCell(z - 1, nextCell.getCellTypeEnum());
                    cloneCell(newCell, nextCell);
                }

            }
        }while (rows.hasNext());
        for(int z = 0; z < maxColumn; z++)
            sheet.setColumnWidth(z, sheet.getColumnWidth(z + 1));
    }*/
}
